// ===========================================
// APP TIENDA CLIENTES - VERSIÓN FINAL CON LOGIN
// ===========================================
...
function verificarSesion() {
    const usuarioActual = Usuarios.obtenerUsuarioActual();
    
    if (usuarioActual) {
        // Usuario ya logueado, actualizar clienteActual
        App.clienteActual = {
...
window.procesarLogin = function() {
    ...
    const resultado = Usuarios.iniciarSesion(telefono);
    
    mensajeDiv.style.display = 'block';
    mensajeDiv.style.background = resultado.exito ? '#d4edda' : '#f8d7da';
    mensajeDiv.style.color = resultado.exito ? '#155724' : '#721c24';
    ...
    if (resultado.exito) {
        ...
        setTimeout(() => {
            ...
            // Inicializar la app después del login
...
// Procesar registro
window.procesarRegistro = function() {
    const nombre = document.getElementById('regNombre').value.trim();
    const telefono = document.getElementById('regTelefono').value.trim();
    const direccion = document.getElementById('regDireccion').value.trim();
    const mensajeDiv = document.getElementById('loginMensaje');
    
    const resultado = Usuarios.crearUsuario(nombre, telefono, direccion);
    ...
    if (resultado.exito) {
        ...
        setTimeout(() => {
            ...
            // Llenar teléfono automáticamente
            document.getElementById('loginTelefono').value = telefono;
...
window.cambiarTab = function(tab) {
    ...
    if (tab === 'login') {
        ...
        tabRegistro.style.borderBottomColor = 'var(--primary-color)';
        tabRegistro.style.color = 'var(--primary-color)';
...
window.cerrarSesion = function() {
    ...
    // Limpiar campos
    document.getElementById('loginTelefono').value = '';
    
    // Vaciar datos sensibles
    App.carrito = [];
    App.favoritos = [];
...
async function initApp() {
    ...
    if (!tieneSesion) {
        ...
        // No mostrar página principal hasta que inicie sesión
        return;
    }
...
function mostrarMenuLateral() {
    ...
    if (menuExistente) {
        ...
        const overlayExistente = document.getElementById('menuOverlay');
        ...
        <div style="padding: 1rem;">
            ...
            <div style="margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #eee;">
                ...
                <div style="font-size: 0.8rem; color: #666; line-height: 1.6;">
                    ...
                    ${App.clienteActual.direccion ? `<p>🏠 ${App.clienteActual.direccion}</p>` : ''}
                    <p style="margin-top: 0.5rem;">🕒 Última actualización: ${new Date().toLocaleTimeString()}</p>
                </div>
            </div>
            
            <div style="margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #eee;">
                <button onclick="cerrarSesion()" style="width: 100%; padding: 1rem; background: #e74c3c; color: white; border: none; border-radius: 8px; font-size: 1rem; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
    ...
    return `
        <div class="page active">
            <div class="dashboard-bienvenida" style="text-align: center; padding: 1rem; margin-bottom: 1rem; background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: white; border-radius: var(--border-radius);">
                <h2 style="margin-bottom: 0.5rem;">¡Bienvenido, ${App.clienteActual.nombre}!</h2>
        ...
        html += `
            <div class="vale-compra" style="background: white; border-radius: var(--border-radius); margin-bottom: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.1); overflow: hidden; border: 1px solid #eee;">
                ...
                <div style="padding: 0.8rem; background: #f8f9fa; border-bottom: 1px solid #eee;">
                    <strong>Cliente:</strong> ${compra.cliente ? compra.cliente.nombre : App.clienteActual.nombre}
                </div>
...
function setupEventListeners() {
    ...
    // Event listeners para pestañas de login
    const tabLogin = document.getElementById('tabLogin');
    const tabRegistro = document.getElementById('tabRegistro');
    ...
    if (loginTelefono) {
        loginTelefono.addEventListener('input', function(e) {
    ...
    if (loginTelefono) {
        loginTelefono.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                procesarLogin();
            }
        });
